...
function deploy(yargs, argv) {
  ...
  if (argv2.r) {
    ...
    try {
      var llamaConfig = JSON.parse(fs.readFileSync('./chaos_lambda_config.json', 'utf8'));
      role = llamaConfig.LambdaRoleArn;
    } catch (err) {
      console.log(chalk.red('chaos_lambda_config.json not found'), '- please specify the lambda role ARN to use with -r');
      return;
    }
  }
  ...
  createLambda(role, llamaFile, function(err, data) {
    ...
    debug(data);
    fs.writeFileSync('chaos_lambda_config.json',
                     JSON.stringify({
    ...
    if (llamaFile) {
      ...
      configSchedule(interval, llamaConfig.FunctionArn, function(err) {
        ...
        console.log(chalk.green(util.format('+ Chaos Lambda\'s schedule updated to once every %s minutes', interval)));
      });
    }
  });
}
...
function createLambda(role, llamaFile, done) {
  ...
  tmpfStream.on('finish', function() {
      ...
      Code: {
        ...
        Role: role,
        Runtime: 'nodejs6.10',
        Description: 'Chaos Lambda - https://github.com/shoreditch-ops/chaos-lambda',
        MemorySize: 128,
        Publish: true,
        Timeout: 300
      };
...
function createEventRule(context, next) {
  ...
  var params = {
    ...
    Description: 'Schedule for invoking Chaos Lambda - https://github.com/shoreditch-ops/',
